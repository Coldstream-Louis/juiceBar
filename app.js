...
app.get('/seasonal.html', function (req, res) {
  res.sendFile(path.join(__dirname, 'views/seasonal.html'));
});

app.get('/standard.html', function (req, res) {
  res.sendFile(path.join(__dirname, 'views/standard.html'));
});

app.get('/standardsmoothie.html', function (req, res) {
  res.sendFile(path.join(__dirname, 'views/standardsmoothie.html'));
});
...
app.get('/yourCart.html', function (req, res) {
  res.sendFile(path.join(__dirname, 'views/yourCart.html'));
});

app.get('/successfulPayment.html', function (req, res) {
  res.sendFile(path.join(__dirname, 'views/successfulPayment.html'));
});

app.get('/diy', function (req, res) {
  res.sendFile(path.join(__dirname, 'views/diy.html'));
});
...
Data.prototype.makeTransaction = function(order, changeUnit) {
    ...
    console.log("drinks: ");
    console.log(drinks);
    var test = drinks[0].ingredients;
    console.log("test: ");
    console.log(test);

    for(var m = 0; i < drinks.length; m++){
        var drink = drinks[m];
        console.log("drink: ");
        console.log(drink);
...
Data.prototype.addOrder = function (order) {
  this.orders[order.orderId] = {};
  this.orders[order.orderId].order = order.order;
...
io.on('connection', function (socket) {
  ...
  socket.on('orderIsDone', function (orderId) {
